...
namespace sld {
    ...
    SLD_MEMORY_API block_allocator* block_memory_init           (const u32 block_count, const u32 block_size);
    SLD_MEMORY_API u32              block_size                  (const block_allocator* alctr);
    SLD_MEMORY_API u32              block_allocator_count_total (const block_allocator* alctr);
    SLD_MEMORY_API u32              block_allocator_count_free  (const block_allocator* alctr);
    SLD_MEMORY_API u32              block_allocator_count_used  (const block_allocator* alctr);
    SLD_MEMORY_API u32              block_allocation_size       (const void* block);
    SLD_MEMORY_API u32              block_allocation_id         (const void* block);
    SLD_MEMORY_API u64              block_allocation_timestamp  (const void* block);
